...
Vagrant.configure("2") do |config|
  ...
  if data['vm']['hostname'].to_s != ''
    ...
    config.vm.network "private_network", ip: "172.16.2.100"
  ...
  data['vm']['synced_folder'].each do |i, folder|
    if folder['source'] != '' && folder['target'] != '' && folder['id'] != ''
      ...
      group = (folder['group']) ? folder['group'] : "vagrant"
      user = (folder['user']) ? folder['user'] : "vagrant"
      config.vm.synced_folder "#{folder['source']}", "#{folder['target']}", id: "#{folder['id']}", type: nfs, :owner=>group, :group=>group
    end
  end
