...
class VehicleStyleFavoritable extends Favoritable implements FavoritableInterface
{
    ...
    public function isFavorite(\SOE\Persons\PersonInterface $person)
    {
        ...
        return $this->favorites->findByUserTypeId($person->getId(), 'VehicleStyle', $this->model->id);
    }
    ...
    public function unfavorite(\SOE\Persons\PersonInterface $person)
    {
        ...
        $favorite = $this->favorites->findByUserTypeId($person->getId(), 'VehicleStyle', $this->model->id);
        if(!$favorite)
            return false;
