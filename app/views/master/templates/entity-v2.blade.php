...
@if($entity->entitiable_type == 'Offer')
    @if(($entity->secondary_type == 'lease') || ($entity->secondary_type == 'purchase'))
        <div class="item {{$entity->secondary_type}} version2" itemscope itemtype="http://schema.org/Organization">
            <div class="item-type {{$entity->secondary_type}}"></div>
            <div class="top-pic" style="background-image: url('{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}');">
                <a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug.'/'}}{{$entity->merchant_slug.'/'.$entity->location_id}}">
                    <img alt="{{$entity->merchant_name}}" class="img-responsive" src="{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}">
                </a>
            </div>
              ...
              <div itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                <a class="item-info" href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug.'/'}}{{$entity->merchant_slug.'/'.$entity->location_id . '?showeid=' . $entity->id}}" itemscope itemtype="http://schema.org/Product">
                    <div class="item-name" itemprop="name">
                      @if($entity->short_name_line1 != '')
                        <div class="line1">
                          {{$entity->short_name_line1}}
                        </div>
                        <div class="line2">
                          {{$entity->short_name_line2}}
                        </div>
                      @else
                        <div class="{{strlen($entity->name)<=25?'line1':''}}">
                          {{$entity->name}}
                        </div>
                      @endif
                    </div>
                </a>
                <p class="category-links"><a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}">{{SoeHelper::unSlugCategory($entity->category_slug)}}</a> > <a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug}}">{{SoeHelper::unSlugCategory($entity->subcategory_slug)}}</a></p>
                @if(!$entity->hide_expiration)
                <p class="expires_at" itemprop="availabilityEnds">Expires {{$entity->is_reoccurring ? date('m/t/Y') : date('m/d/Y', strtotime($entity->expires_at))}}</p>
                @else
                <div class="expires-at-spacer"></div>
                @endif
            ...
            <div class="btn-group">
                ...
                <button type="button" class="btn btn-default btn-save-coupon {{$entity->is_clipped == true ? 'disabled' : ''}}" data-offer_id="{{$entity->entitiable_id}}" data-entity_id="{{$entity->id}}" data-entity_id="{{$entity->id}}" data-toggle="tooltip" data-placement="top" title="Save This Lease Special for {{$entity->name}} from {{$entity->merchant_name}}"><img src="{{$entity->is_clipped == true ? 'http://saveoneverything_assets.s3.amazonaws.com/assets/images/icons/icon-burgundy-saved.png' : 'http://saveoneverything_assets.s3.amazonaws.com/assets/images/icons/icon-burgundy-save-it.png'}}" alt="Save This Lease Special for {{$entity->name}} from {{$entity->merchant_name}}"></button>
    ...
    @elseif($entity->is_dailydeal == '1')
        <div class="item save-today {{ $entity->is_certified ? 'save_certified_offer' : ''}} version2" itemscope itemtype="http://schema.org/Organization">
            <div class="item-type save-today"></div>
            <div class="top-pic" style="background-image: url('{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}');">
                <a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug.'/'}}{{$entity->merchant_slug.'/'.$entity->location_id}}">
                    <img alt="{{$entity->merchant_name}} Coupons" class="img-responsive" src="{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}">
                </a>
            </div>
            <a class="item-info" href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug.'/'}}{{$entity->merchant_slug.'/'.$entity->location_id . '?showeid=' . $entity->id}}"itemscope itemtype="http://schema.org/Product">
              <span class="hidden" itemprop="name">{{$entity->merchant_name}} - {{$entity->name}}</span>
              ...
              <div itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                ...
                <p class="category-links"><a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}">{{SoeHelper::unSlugCategory($entity->category_slug)}}</a> > <a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug}}">{{SoeHelper::unSlugCategory($entity->subcategory_slug)}}</a></p>
                @if(!$entity->hide_expiration)<p class="expires_at" itemprop="availabilityEnds">Expires {{$entity->is_reoccurring ? date('m/t/Y') : date('m/d/Y', strtotime($entity->expires_at))}}</p>@endif
                <span itemprop="price" class="hidden">{{$entity->special_price}}</span>
              </div>
              <p class="merchant-name" itemprop="name">{{$entity->merchant_name}}</p>
            @if($entity->company_id > 1)
    ...
    @else
        <div class="item coupon {{$entity->is_certified == 1 ? 'save_certified_offer' : ''}} version2" itemscope itemtype="http://schema.org/Organization">
            <div class="item-type coupon"></div>
            <div class="top-pic {{($entity->company_id == 2)?'yipit':''}}" style="background-image: url('{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}');">
                <a href="{{URL::abs('/')}}/coupons/{{strtolower($geoip->region_name)}}/{{SoeHelper::getSlug($geoip->city_name)}}/{{$entity->category_slug}}/{{($entity->category_slug == $entity->subcategory_slug)?'':$entity->subcategory_slug.'/'}}{{$entity->merchant_slug.'/'.$entity->location_id}}">
                    <div class="expired-banner"><span class="h3 spaced">Expired</span></div>
                    <img alt="{{$entity->merchant_name}} Coupons" class="img-responsive" src="{{$entity->path != $entity->logo ? $entity->path : ($entity->about_img ? $entity->about_img : $entity->path)}}" itemprop="image">
                </a>
            </div>
            <div itemscope itemtype="http://schema.org/Product">
              <span class="hidden" itemprop="name">{{$entity->merchant_name}} - {{$entity->name}}</span>
...
@elseif($entity->entitiable_type == 'Contest')
    <div class="item contest btn-get-contest version2" data-offer_id="{{$entity->entitiable_id}}" data-entity_id="{{$entity->id}}" itemscope itemtype="http://schema.org/Organization">
      <div itemscope itemtype="http://schema.org/Product">
